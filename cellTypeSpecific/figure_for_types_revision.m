function figureForTypes = figure_for_types_revision( Ref, volumes, options )
...
if nargin < 3
    ...
    for vv = 1 : numTypes
        options{ vv } = struct( 'regionChoice', 'maxAverage', 'sectionStyle', 'coronal',...
            'identifierIndex', 5, 'isCorrelation', 1, 'isFitting', 0, 'fontSize', 20, 'fontSizeAtlas', 14, 'regionIndicesToPick', 1 : 13,...
            'customIndex', 1, 'desiredIndex', 36, 'corMult', 0.87, 'stripDim', [ 200 200 1385 255], 'labelForType', '',...
            'xPosProj', [ 0.01, 0.18, 0.33, 0.48, 0.62, 0.82, 0.78] );
    end
end
cols = { '-b', '-g', '-c', '-g*', '-b*', '-c*', '-go', '-bo', '-co', '-g.', '-b.', '-c.', '-gs' };
colorCodes = { [ 0, 1, 0 ], [ 0.2, 0.3, 1 ], [ 0.2, 0.6, 1 ], [ 0.5, 1, 1 ], [ 0.5, 0, 1 ], [ 0.5, 1, 0 ],...
                [ 0.5, 1, 0.5 ], [ 1, 0, 1 ], [ 0, 1, 0.5  ], [ 0.5, 0.5, 1 ], [ 0.5, 0, 1 ], [ 0.5, 0.1, 0.6 ], [ 0, 1, 1 ] } ;   
...
set( gca,'Color','k','Visible','on',...
        'XTick',[],'YTick',[],'DataAspectRatio',[1 1 1]);
    
% lineLength = numTypes * options{ vv }.stripDim( 3 ) * 0.85;
% xl( 1 ) = options{ vv }.stripDim( 3 ) * 0.25;
% xl( 2 ) = xl( 1 );
% yl( 1 ) = lineLength;
% yl( 2 ) = 10;
% axes('Position', [ 0.41 0.05 0.1 0.9 ],'Visible','on');
% plot( xl, yl, 'linewidth', 10, 'color', 'w' );    
% set( gca,'Color','k','Visible','on',...
%         'XTick',[],'YTick',[],'DataAspectRatio',[1 1 1]); 
...
yl( 1 ) = lineLength;
yl( 2 ) = 10;
axes('Position', [ 0.42 0.02 0.02 0.96 ],'Visible','off');
plot( [ 54, 54 ], [ 100, 200], 'linewidth', 6, 'color', 'w' );
...
for vv = 1 : numTypes
    ...
    set( hleg, 'Position', [ xPosProj( 6 ), bottomDist( vv )+0.05, widthParam, heightParam * 0.4 ], 'fontweight', 'b', 'fontsize', fontSizeAtlas, 'Color', 'w' );
    %pause;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
axes('Position',[0 0 1 1],'Visible','off');
h = text( xPosProj( 2 )+0.07, 0.98, 'Maximal-intensity projections', 'color', 'w', 'FontSize', fontSize + 4, 'HorizontalAlignment','center');
h = text( xPosProj( 1 )+0.07, 0.94, 'Sagittal', 'color', 'w', 'FontSize', fontSize, 'HorizontalAlignment','center');
h = text( xPosProj( 2 )+0.07, 0.94, 'Coronal', 'color', 'w', 'FontSize', fontSize, 'HorizontalAlignment','center');
h = text( xPosProj( 3 )+0.08, 0.94, 'Axial', 'color', 'w', 'FontSize', fontSize, 'HorizontalAlignment','center');
h = text( xPosProj( 4 )+0.09, 0.98, 'Plane of section', 'color', 'w', 'FontSize', fontSize + 3, 'HorizontalAlignment','center');
h = text( xPosProj( 5 )+0.08, 0.98, 'Section', 'color', 'w', 'FontSize', fontSize + 3, 'HorizontalAlignment','center');
...
numTypesShifted = numTypes * 1.08;
 ...
 for vv = 1 : numTypes
    labelForType = options{ vv }.labelForType;
    h = text( 0.02, bottomDist( vv ) + 0.05, options{ vv }.labelForType, 'color', 'w',...
